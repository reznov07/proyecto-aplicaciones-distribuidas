...
Nota de estado: este documento describe la arquitectura tal como está implementada a la fecha. Hay un punto pendiente marcado explícitamente en la sección 3 (cierre del ciclo con el evento voto.completado) que aún no está implementado en la lógica del código, pero la infraestructura y resiliencia están 100% operativas.
...
El enunciado pide un punto único de acceso. En este proyecto ese rol lo cumple el Ingress de Traefik (incluido por defecto en K3s), que expone un solo dominio por ambiente (qa.grupo6.uta.cl / prod.grupo6.uta.cl) y enruta por path hacia el frontend y los backends, sin exponer puertos individuales. No se construyó un microservicio "API Gateway" independiente porque el Ingress ya resuelve el requisito de acceso único sin agregar un salto de red adicional que no aporta lógica de negocio.

3. Flujo de Negocio (estado actual)

El ciudadano abre el frontend, que consulta la elección activa y sus candidatos.

El ciudadano envía ciudadanoId y candidatoId a POST /api/sufragio/votar.

Gestión de Sufragio crea la sesión en estado INICIADO y responde 202 Accepted.

Se publica el evento voto.intento_verificar.

Padrón Electoral verifica elegibilidad (existe / habilitado / no ha votado) y registra la participación en votos_efectuados.

Se publica el evento voto.verificado_anonimo con el resultado.

En paralelo: Gestión de Sufragio resuelve la sesión (APROBADO/RECHAZADO) y Registro de Auditoría guarda el registro anónimo.

El frontend hace polling de GET /api/sufragio/estado/:sesionId hasta obtener el resultado final.
...
sesiones_sufragio: id (PK), ciudadano_id, eleccion_id, estado, motivo_resultado, fecha_resolucion.

elecciones: id, nombre, activa, fecha_inicio, fecha_fin.

candidatos: id, candidato_id, eleccion_id (FK), nombre, partido.

5.2 db_padron (servicio-padron)

ciudadanos: id, ciudadano_id (único), nombre, apellido, habilitado, motivo_inhabilitacion, elecciones_habilitadas.

historial_habilitacion: id, ciudadano_id, accion, motivo, operador.

votos_efectuados: id, ciudadano_id, eleccion_id, sesion_id, fecha_participacion. UNIQUE(ciudadano_id, eleccion_id) impide doble voto.

5.3 db_auditoria (servicio-auditoria)

registros_auditoria: id, sesion_id (único), eleccion_id, candidato_id, resultado, motivo, evento_origen_id, fecha_registro.

6. Guía de Acceso (QA / PROD en el clúster K3s)
...
Abra el archivo C:\Windows\System32\drivers\etc\hosts.
...
Ejecute en terminal: sudo nano /etc/hosts
...
7. Manual Operativo (Auditoría y Resiliencia)

Comandos indispensables para verificar la salud y resiliencia del sistema desplegado (reemplazar <ns> por grupo6-qa o grupo6-prod según corresponda):

7.1 Auditoría de Persistencia y Copias de Seguridad (CronJobs)

El sistema ejecuta respaldos automáticos intra-clúster de las 3 bases de datos cada 10 minutos, conservando solo las últimas 6 copias (rotación inteligente). Para auditar esto en vivo:

Verificar la programación de los CronJobs:

kubectl get cronjobs -n grupo6-prod


Certificar el almacenamiento físico (Ejemplo DB Padrón):
Obtenga el nombre del pod de la base de datos y revise el volumen persistente montado:

# Obtener nombre del pod
kubectl get pods -l app=postgres-padron -n grupo6-prod

# Listar copias de seguridad sobrevivientes
kubectl exec -it <NOMBRE_POD_POSTGRES_PADRON> -n grupo6-prod -- ls -lh /backups


7.2 Trazas Unificadas (Logging Centralizado)

Para observar el flujo asíncrono de los JSON viajando por RabbitMQ sin tener que saltar de contenedor en contenedor, ejecute este comando para seguir todos los logs de los microservicios productivos en tiempo real:

kubectl logs -f -l tier=backend --all-containers=true -n grupo6-prod


(Nota: Este comando agrupa el historial de eventos delegando el diagnóstico de cuellos de botella).

7.3 Verificar RabbitMQ y Colas

kubectl -n grupo6-prod port-forward svc/rabbitmq 15672:15672
# Abrir http://localhost:15672 (usuario/clave en secrets.yaml)
